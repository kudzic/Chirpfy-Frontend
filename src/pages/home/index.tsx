...
const Home = () => {
  ...
  const handleChangeTweet = (): void => {
    let tweetRef = tweetTextareaRef.current;
    if (tweetRef) {
      let tweetText: string = tweetRef.value;
      setTweetLength(tweetText.length);
      tweetText.length
        ? setDisableTweetButton(false)
        : setDisableTweetButton(true);
      // setCommentText(commentText);
      tweetTextareaRef.current.style.height = tweetTextareaRef.current.scrollHeight + "px";
    }
  };

  const tweet = () => {
    const tweetText = tweetTextareaRef.current?.value;
    if (!tweetText) {
    ...
    const post = {
      url: `http://localhost:8083/api/user/addTweet?email=${email}`,
      method: "POST",
      headers: {
